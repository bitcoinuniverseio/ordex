...
for (const size of [1, 2, 3, 4, 5, 6, 7, 8]) {
  test(`a ${size} member collection roots and proves every member`, () => {
    ...
    const manifest = { ...body, digest: collectionManifestDigest(body) };
    assert.equal(verifyCollectionManifest(manifest).ok, true);
    for (const member of members) {
      const proof = buildMembershipProof(collectionId, members, member);
      const verdict = verifyMembershipProof({ manifest, memberIdentity: member, proof });
...
test('a member removed from the list cannot prove membership', () => {
  ...
  const manifest = { ...body, digest: collectionManifestDigest(body) };
  const verdict = verifyMembershipProof({ manifest, memberIdentity: 'b', proof: staleProof });
