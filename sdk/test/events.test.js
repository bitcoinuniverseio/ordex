...
test('the signer refuses to sign nonsense instead of producing a weak header', () => {
  assert.throws(() => signWebhookDelivery({ secret: '', timestamp: 10, deliveryId: 'd', body: 'b' }), TypeError);
  assert.throws(() => signWebhookDelivery({ secret: 's', timestamp: 10.5, deliveryId: 'd', body: 'b' }), TypeError);
  assert.throws(() => signWebhookDelivery({ secret: 's', timestamp: 10, deliveryId: 'd', body: { json: true } }), TypeError);
});
